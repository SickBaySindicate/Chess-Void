...
void ChessVoid::makevoid(const Square &sq) noexcept {
    assert(piece_on(sq) == Piece::None);
    blank_.set(sq);
}

void ChessVoid::makemove(const Move &move) noexcept {
    const auto us = turn();
    ...
    history_.push_back(
        meh{
            hash_old, move, ep_old, halfmove_clock_old, blank_, {castling_[0], castling_[1], castling_[2], castling_[3]}
        });

    // Castling permissions
    castling_[usKSC] &=
            !((piece == Piece::King && us == Side::White) || from == castle_rooks_from_[0] || to == castle_rooks_from_[
                  0]);
    castling_[usQSC] &=
            !((piece == Piece::King && us == Side::White) || from == castle_rooks_from_[1] || to == castle_rooks_from_[
                  1]);
    castling_[themKSC] &=
            !((piece == Piece::King && us == Side::Black) || from == castle_rooks_from_[2] || to == castle_rooks_from_[
                  2]);
    castling_[themQSC] &=
            !((piece == Piece::King && us == Side::Black) || from == castle_rooks_from_[3] || to == castle_rooks_from_[
                  3]);
...
void ChessVoid::undomove() noexcept {
    // Swap sides
    to_move_ = !to_move_;
